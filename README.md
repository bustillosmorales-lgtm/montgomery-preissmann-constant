...
> F. Bustillos, *A sharp one-sided uncertainty principle, the constant in the Montgomery–Preissmann conjecture, and Toeplitz matrices with a zero at a jump*.
...
which settles the conjecture of Montgomery and Preissmann and identifies its constant, together with a one-sided uncertainty principle with sharp exponent `pi^2` and its consequences for Toeplitz matrices whose symbol vanishes on one side of a jump.
...
| `code/check_upper.py` | The Gaussian packets: exact first moment, leak bound, and the resulting upper bound. |
...
python code/verify_all.py            # 107 checks against results/
```

`verify_all.py` prints one line per quoted value, with the value in the article and the value recomputed from `results/`, and ends with the number of failures. On the stored results it reports `107 checks, 0 FAIL`.
